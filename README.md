...
*   **웹 프레임워크**: `Streamlit`
*   **기타**: `python-dotenv`, `pandas`, `screen_brightness_control`
...
이 프로젝트는 기능별로 코드를 모듈화하여 관리의 용이성과 재사용성을 높였습니다. 각 파일의 주요 역할은 다음과 같습니다.

*   `app.py`: **메인 애플리케이션 파일**
    *   Streamlit 앱의 진입점(entry point)입니다.
    *   카메라를 초기화하고, 각 모듈의 클래스(`VisionManager`, `VitalSignsProcessor` 등)를 조율하여 전체 프로세스를 관리합니다.
    *   실시간 루프를 통해 프레임 획득, 데이터 처리, UI 업데이트 등 모든 작업을 오케스트레이션합니다.

*   `vision.py`: **컴퓨터 비전 및 신호 추출 모듈**
    *   `VisionManager` 클래스가 포함되어 있습니다.
    *   `MediaPipe`를 사용하여 카메라 프레임에서 얼굴과 자세 랜드마크를 실시간으로 감지합니다.
    *   감지된 얼굴 영역(이마)의 미세한 색상 변화를 분석하여 원격 광용적맥파(rPPG) 신호를 추출하고 버퍼에 저장합니다.

*   `processing.py`: **생체 신호 처리 모듈**
    *   `VitalSignsProcessor` 클래스가 포함되어 있습니다.
    *   `vision.py`에서 추출된 rPPG 신호를 입력받아 `Scipy`와 `Numpy`를 이용해 필터링 및 분석합니다.
    *   심박수(FFT), 산소포화도(RGB 비율), 호흡수(저주파 분석), 스트레스 지수(HRV 분석) 등 핵심 건강 지표를 계산합니다.

*   `ui.py`: **사용자 인터페이스(UI) 모듈**
    *   Streamlit을 사용하여 웹 애플리케이션의 전체 레이아웃을 구성합니다.
    *   `st.metric`과 같은 컴포넌트를 사용하여 계산된 건강 지표, 자세 분석 결과, AI 조언 등을 실시간으로 시각화하고 업데이트하는 함수들을 포함합니다.

*   `utils.py`: **유틸리티 및 외부 API 연동 모듈**
    *   `EnvironmentOptimizer` 클래스와 AI 조언 생성 함수가 포함되어 있습니다.
    *   랜드마크 데이터를 기반으로 거북목 각도와 화면 시청 거리를 계산합니다.
    *   `OpenAI (GPT-4o)` API와 연동하여 측정된 건강 데이터에 기반한 맞춤형 조언을 생성합니다.
    *   시간대에 따라 화면 밝기를 조절하는 등의 부가 기능을 담당합니다.
